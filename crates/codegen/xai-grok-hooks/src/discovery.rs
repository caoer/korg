...
/// url_raw, matcher_dedupe_key) so a hook from several origins runs once; earlier
...
/// configs cannot double-register the same body. Distinct matchers that truly
/// target different tools still keep separate entries. The canonical event
/// collapses aliases (`SubagentStop`/`SubagentEnd`).
